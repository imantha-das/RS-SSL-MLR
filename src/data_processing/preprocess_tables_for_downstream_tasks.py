...
def prioratise_and_select_imgp(df:pd.DataFrame, drn_repo_p:str, sat_repo_p:str):
    ...
    for i in range(df.shape[0]):
        ...
        sat_img_list = ast.literal_eval(row["sen2a_c3_256x_ext"])
    ...
    df.drop(["drn_c3c4_256x_ext", "sen2a_c3_256x_ext"], axis = 1, inplace = True)
...
if __name__ == "__main__":
    ...
    feat_cols_ignore = [
        ...
        "mergeID", "hadMalaria","houseID","individualID","goForest","GPS_X", "GPS_Y", "pf","pv",
    ...
    df_imgp = pd.read_csv("data/interim/sshsph_mlr/mlr_geopts_imgp_v1.csv")
    ...
    df = prioratise_and_select_imgp(
        df, 
        drn_repo_p="data/interim/sshsph_drn/drn_c3c4_256x_ext",
        sat_repo_p = "data/interim/gee_sat/sen2a_c3_256x_ext"
    )
    print(df.info())
    
    #! There are only 700ish entries. Check which cols to drop to maximize dataset
    df.to_csv("data/processed/sshsph_mlr/mlr_data_nomiss_some_var_dropped.csv", index= False)
