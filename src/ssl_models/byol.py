...
class ByolBBResnet(pl.LightningModule):
    ...
    def __init__(self, model_params:dict, backbone:torch.nn.Sequential):
        ...
        self.prediction_head = BYOLPredictionHead(
            input_dim = model_params["proj_output_dim"],
            hidden_dim = model_params["pred_hidden_dim"],
            output_dim = model_params["pred_output_dim"]
        ) # 256 > 4096 > 256
    ...
    def configure_optimizers(self):
        if self.model_params["apply_lr_scheduler?"]:
            ...
            if self.model_params["eff_batch_size"] < 1000:
                optimizer = torch.optim.SGD(params= self.parameters(), lr = self.model_params["base_lr"], weight_decay= self.model_params["weight_decay"])  
            else:
                print(colored("Using LARS optimizer", "green"))
                optimizer = LARS(params = self.parameters(), lr = self.model_params["base_lr"], weight_decay = self.model_params["weight_decay"])
            
            self.scheduler = LinearWarmupCosineAnnealingLR(
                optimizer = optimizer, 
                warmup_epochs= self.model_params["scheduler_warmup_epochs"], # Linearly rampup lr as then decay using cosine as indicated in paper
                max_epochs= self.model_params["epochs"], # Should be 1000 if we training for that long but we arnt
                warmup_start_lr= self.model_params["scheduler_warmup_start_lr"], #* we linearly ramp up from 0 to base_lr, start value not indicated
                eta_min=self.model_params["scheduler_eta_min"] #* We keep eta_min at 0 as byol Paper hasnt indicated a value
            )
