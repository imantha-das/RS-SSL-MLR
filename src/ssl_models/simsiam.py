...
class SimSiamBBResnet(pl.LightningModule):
    ...
    def __init__(self, model_params:dict, backbone:torch.nn.Sequential):
        ...
        self.prediction_head = SimSiamPredictionHead(
            input_dim = model_params["proj_output_dim"], # input dims for prediction head is the same as the ouput dims
    ...
    def configure_optimizers(self):
        if self.model_params["apply_lr_scheduler?"]:
            lr = self.model_params["base_lr"] * self.model_params["batch_size"] / 256
